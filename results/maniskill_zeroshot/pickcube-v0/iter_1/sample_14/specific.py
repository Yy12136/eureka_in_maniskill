...
def compute_dense_reward(self, action) -> float:
    ...
    # 2. Distance to cube reward (negative distance to encourage approaching the cube)
    distance_to_cube = np.linalg.norm(self.tcp.pose.p - self.obj.pose.p)
    reward_distance_to_cube = -distance_to_cube
    
    # 3. Distance to goal reward (negative distance to encourage moving the cube to the goal)
