...
def compute_dense_reward(self, action) -> float:
    ...
    # 2. Distance to goal reward (negative distance to encourage minimizing distance)
    distance_to_goal = np.linalg.norm(self.obj.pose.p - self.goal_pos)
    reward_distance_to_goal = -distance_to_goal
    
    # 3. Action regularization (penalize large actions for smoother control)
    ...
    if distance_to_goal < 0.01:  # Threshold for task completion
