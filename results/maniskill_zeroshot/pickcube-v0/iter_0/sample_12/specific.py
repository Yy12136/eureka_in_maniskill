...
def compute_dense_reward(self, action) -> float:
    ...
    weight_distance_to_goal = 0.5 # Reward for minimizing distance to goal
    weight_action_regularization = 0.2 # Regularization for smooth actions
    # Note: weight_grasp + weight_distance_to_goal + weight_action_regularization = 1.0
    
    # Initialize reward components (total number <= 5)
    reward_grasp = 0.0          # Reward for successful grasp
    reward_distance_to_goal = 0.0 # Reward for minimizing distance to goal
    reward_action_regularization = 0.0 # Regularization for smooth actions
