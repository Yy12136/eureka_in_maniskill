...
def compute_dense_reward(self, action) -> float:
    ...
    reward_reach = max(0, 1 - tcp_to_cube_dist / 0.1)  # Normalized reward based on distance
    ...
    if self.agent.check_grasp(self.obj):
        ...
        target_height = 0.2
        reward_lift = max(0, 1 - abs(cube_height - target_height) / 0.1)  # Normalized reward based on height
