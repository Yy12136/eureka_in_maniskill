...
def compute_dense_reward(self, action) -> float:
    ...
    reward_reach = max(0, 1 - tcp_to_cube_dist / 0.1)  # Normalized distance reward
    
    # 2. Reward for successfully grasping the cube
    if self.agent.check_grasp(self.obj):
        reward_grasp = 1.0
    
    # 3. Reward for lifting the cube to the desired height (0.2 meters)
    if self.agent.check_grasp(self.obj):
        cube_height = self.obj.pose.p[2]
        target_height = 0.2
        reward_lift = max(0, 1 - abs(cube_height - target_height) / 0.1)  # Normalized height reward
