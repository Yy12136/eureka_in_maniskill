...
def compute_dense_reward(self, action) -> float:
    ...
    reward_task_progress = max(0, (current_qpos - target_qpos) / target_qpos)
    
    # 2. Handle Reach: Reward based on the distance between the end-effector and the handle
    end_effector_pos = self.agent.hand.pose.p
    handle_pos = self.target_link.pose.p
    distance_to_handle = np.linalg.norm(end_effector_pos - handle_pos)
    ...
    # 1. Bonus for maintaining the drawer above a certain height (if applicable)
