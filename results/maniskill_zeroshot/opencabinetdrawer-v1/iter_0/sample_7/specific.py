...
def compute_dense_reward(self, action) -> float:
    ...
    # 2. Grasp Success: Reward if the gripper is close to the handle
    ...
    reward_grasp_success = 1.0 - min(1.0, distance_to_handle / 0.1)  # Normalize to [0, 1]
    
    # 3. Motion Control: Penalize large joint velocities for smooth motion
    joint_velocities = self.agent.robot.get_qvel()[:-2]
    reward_motion_control = 1.0 - min(1.0, np.linalg.norm(joint_velocities) / 5.0)  # Normalize to [0, 1]
    ...
    # 1. Bonus for achieving the target drawer position
