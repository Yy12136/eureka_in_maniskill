...
def compute_dense_reward(self, action) -> float:
    ...
    # Note: weight_task_progress + weight_grasp + weight_effort = 1.0
    ...
    reward_task_progress = max(0.0, (self.current_angle / self.target_angle))
    
    # 2. Grasp state: reward for maintaining a grasp on the handle
    if self.agent.check_grasp(self.target_link):
        reward_grasp = 1.0
    
    # 3. Motion control: penalize large actions to encourage smooth motion
    reward_effort = -0.01 * sum(abs(action))
