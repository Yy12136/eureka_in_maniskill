...
def compute_dense_reward(self, action) -> float:
    ...
    if self.agent.check_grasp(self.target_link):
        ...
        reward_grasp = 0.0  # No reward if the handle is not grasped
    ...
    if current_angle >= target_angle:
        reward += 1.0  # Full bonus for task completion
    
    # 2. Penalty for excessive joint velocities
    joint_velocities = self.agent.robot.get_qvel()[:-2]
    velocity_magnitude = sum([v**2 for v in joint_velocities]) ** 0.5
    if velocity_magnitude > 1.0:
        reward -= 0.5  # Penalize high joint velocities
