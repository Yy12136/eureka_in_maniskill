...
def evaluate_reward_function(task_name, reward_path, train_steps, eval_episodes, bo_iter, task_iter, sample_idx, weight_file):
    ...
    cmd = [
        "python", "-u",
        "/home/yy/text2reward/run_metaworld/sac.py",
        "--env_id", task_name,
        "--train_num", "8",
        "--eval_num", "5", 
        "--eval_freq", "16000",
...
def evaluate_samples(samples, iteration, task_name, train_max_steps):
    ...
    iter_dir = base_path / "metaworld_zeroshot" / task_name.lower() / f"iteration_{iteration}"
    ...
    for sample_idx in selected_samples:
        ...
        no_change_count = 0  # 添加权重变化计数器
        
        # 创建权重变化记录文件
        iter_dir = Path(f"/home/yy/text2reward/results/metaworld_zeroshot/{task_name.lower()}/iteration_{iteration}")
        ...
        for round_idx in range(n_optimization_rounds):
            ...
            next_weights, _ = optimizer.optimize(
                ...
                extra_metrics={
                    'avg_reward': result['avg_reward'],  # 使用平均奖励而不是仅依赖成功率
...
def run_eureka(cfg, task_name, instruction, prompt_template, map_dict, generator=None):
    ...
    # MetaWorld环境统一使用100万步
    if not hasattr(cfg, 'train_max_steps'):
        cfg.train_max_steps = 1_000_000  # 统一使用100万步
    ...
    if isinstance(generator, ZeroShotGenerator):
        base_dir = Path("results") / "metaworld_zeroshot" / task_name.lower()
        last_reward_filename = "last_reward_zeroshot.py"
    else:
        base_dir = Path("results") / "metaworld_fewshot" / task_name.lower()
